...
<Screenshot
  ...
  maxWidth="550px"
  alt="Connection step input in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="400px"
  alt="Connection config variables in Prismatic app"
/>
...
Connections are added to code-native integrations as `connectionConfigVar` type object properties of `configPages`.

See the [Build Code-Native](code-native-integrations.mdx#connections-in-code-native-integrations) article for details.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Configure connection for instance in Prismatic app"
/>
...
You set up an "OAuth 2.0 App" in a third-party service, like Salesforce, Quickbooks, Jira or Dropbox, and your customers can then click a button, enter their credentials in the OAuth provider, and your Prismatic integration is granted the necessary access to perform actions in the third-party service on the customer's behalf.
...
You'll use those to configure your Prismatic integration's connection.
...
Once you have an OAuth 2.0 App configured in a third-party service, add a new connection in your Prismatic integration for the third-party you're integrating with.
...
There's a good chance the URLs are the same for everyone, so are hidden in the Prismatic UI.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Add OAuth connection to integration in Prismatic app"
/>
...
- The connection's `id`.
  ...
  For [user-level](user-level-configuration.mdx) connections, this will be the user-level config variable ID.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Configure OAuth 2.0 connection via Prismatic app"
/>
...
1. Prismatic deletes the access and refresh tokens from its database, so steps can't reference it
1. [Optional] It's not common, but some OAuth 2.0 providers allow you to _revoke_ a token.
   ...
   If a _revocation endpoint_ is present in a component's connection, Prismatic reaches out to that endpoint to revoke the token with the third-party's API.
...
<Screenshot
  ...
  maxWidth="300px"
  alt="Disconnect active OAuth 2.0 connection in Prismatic app"
/>
...
A template can be referenced by an integration you build, or by an integration your customers build using [embedded designer](embedded-designer.mdx).
...
3. If you store your integrations' [YAML definitions](integrations.mdx#yaml-definition) in source control, your YAML will be cleaner if you reference a template instead of including the connection's input fields directly in the YAML.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Add connection template in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Add connection template inputs in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Reference connection template in Prismatic app"
/>

</TabItem>
<TabItem value="codenative">

Connection templates can be used if you [reference an existing component's connection](code-native-integrations.mdx#using-existing-connections-in-code-native) in your code-native integration.
You can do this by adding a `template` property to your connection reference.

```ts title="Reference connection template in code-native connection" {8}
connectionConfigVar({
...
**Note**: Integrations in Prismatic **send** data to third-party apps and services from those IP addresses.
Invoking integrations in Prismatic is name-based (e.g. hooks.prismatic.io) and IPs are subject to change.
