...
<Vimeo title="Embed Prismatic Marketplace in 20 Minutes" video="906041989" />
...
This article details how to embed Prismatic's integration marketplace into your application.
...
<Screenshot
  ...
  alt="Prismatic integration marketplace embedded in your app"
/>
...
<Screenshot
  ...
  alt="Enable popover for Prismatic embedded marketplace"
/>
...
In this example we add a [message event listener](https://developer.mozilla.org/en-US/docs/Web/API/Window/message_event) to listen for the `PrismaticMessageEvent.INSTANCE_CONFIGURATION_LOADED` event.
...
<Screenshot
  ...
  alt="Theming the Prismatic integration marketplace for your app"
/>
...
- **Light** and **Dark** determine what the Prismatic app itself looks like when you use "dark" or "light" mode.
  Each of your team members can configure dark or light mode settings from their [profile settings page](users.mdx#setting-light-or-dark-mode), or they can choose to have Prismatic follow their operating system's dark/light mode theme.

  _Note_: Your customers will not see **Light** or **Dark** themes unless you create customer users for them and they log directly in to Prismatic.
...
Prismatic currently supports any font that is available on [Google Fonts](https://fonts.google.com/).
...
For example, if you would like to translate `common.loading` to a different phrase, but only on the [alert monitors](monitoring-and-alerting.mdx) page, prepend the phrase key with a `PhraseNamespace`, entering `NAMESPACE__PHRASE-KEY` (see [translations](https://github.com/prismatic-io/translations/tree/main/src)):
...
The embedded marketplace listview screen is presented as an iframe.
If you would prefer to present marketplace integrations in your own custom UI elements, you can.
This is helpful if you build some integrations in Prismatic, and others outside of Prismatic - you can display both together using the same UI elements.

To fetch a list of marketplace integrations, use your customer's signed JWT to [query the Prismatic API](embedded-api-requests.mdx) for `marketplaceIntegrations`.
An example of a query you can issue is available in our [example embedded app](https://github.com/prismatic-io/embedded/blob/main/example-embedded-app/pages/examples/custom-ui-elements.tsx).
