---
id: cli-usage
title: How to Use Prismatic's CLI Tool
description: Installing the prism CLI tool allows Prismatic users access to our API via the command line. You can also run CLI commands and get contextual help.
---
...
The CLI tool is built using the [Prismatic API](api/api-overview.mdx), so any action that can be completed through the web app or API can be completed through the CLI tool, as well.
...
Prism's source code is available on [GitHub](https://github.com/prismatic-io/prism), and serves as a great example for how to wrap the [Prismatic API](api/api-overview.mdx).
...
A full list of `prism` commands can be found on the [Prismatic CLI Command Reference](cli/prism.mdx) page.
...
Refresh tokens do _not_ expire, and are used to generate short-lived access tokens for Prismatic's API.
