...
Multiple events can [trigger](integration-triggers.mdx) an execution:
...
If an execution fails, or if you need to re-run an execution, you can [replay](retry-and-replay.mdx) previous executions.
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Instance execution results in Prismatic app"
/>
...
Step results can be fetched from the Prismatic GraphQL API using the `executionResult` query.
...
For more information: [Fetching and Unpacking Step Results](api/using-prismatic-api.mdx#fetching-step-results-through-the-api)
...
<Screenshot
  ...
  alt="Filter instance logs in Prismatic app"
/>
