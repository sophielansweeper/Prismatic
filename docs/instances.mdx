...
An **instance** of an [integration](integrations.mdx) is a copy of an integration that has been configured for a specific [customer](customers.mdx).
When the instance is configured for a customer or by a customer, config variables and connections to third-party apps and services specific to that customer are set by walking through a [configuration wizard](configuration-wizard.mdx).

You can deploy instances of the integrations you create [on behalf of your customers](org-deployed-instances.mdx) or your customers can enable instances themselves through the [integration marketplace](integration-marketplace.mdx).
...
When a [flow](building-integrations.mdx#flows-in-integrations) within an instance is triggered, an [execution](executions.mdx) of your instance's flow runs.
...
Schedule triggers are registered with the Prismatic scheduler and will run at the interval you specify.
...
1. You can set up your integration to run [on a schedule](integration-triggers.mdx#scheduled-triggers)
1. You can invoke them [through a webhook](integration-triggers.mdx#webhook-triggers)
1. You can configure your flow to run [on deployment](integration-triggers.mdx#instance-deploy-trigger) or [on instance removal](integration-triggers.mdx#instance-remove-trigger)
...
**More Information**:
[Webhook Triggers](integration-triggers.mdx#webhook-triggers)
...
<Screenshot
  ...
  alt="Search instances across customers in Prismatic app"
/>
...
As you work with your first few customers and build out an MVP, it may be easiest to [deploy instances yourself](org-deployed-instances.mdx).
...
You can invite your customers to log in to Prismatic and deploy instances themselves.
You can do this by publishing your integration to the [integration marketplace](integration-marketplace.mdx) and then invite the [customer user](users.mdx#managing-customer-users) to your tenant.
...
You can [embed the integration marketplace](embedding-marketplace.mdx) in your app so that your customers can deploy instances without leaving your app.
...
If you want to build a custom UI for deploying instances, you can use the Prismatic embedded SDK to [make API requests](embedded-api-requests.mdx) to the Prismatic API on behalf of your customer user.
...
When you or your customer deploy an instance, you will work through the [Configuration Wizard](configuration-wizard.mdx) that your integration builders created.
...
<Screenshot
  ...
  maxWidth="800px"
  alt="Configure instance in Prismatic app"
/>
