...
Prismatic's [integration-publisher](https://github.com/marketplace/actions/prismatic-integration-publisher) and [component-publisher](https://github.com/marketplace/actions/prismatic-component-publisher) GitHub Actions use the [prism](cli/cli-usage.mdx) CLI tool to publish integrations and components to Prismatic.
To authenticate the GitHub Actions with your Prismatic tenant(s), you'll need to create a Prismatic refresh tokens for each tenant.
...
Prismatic detects if the component has changed since the last publish.
...
- **PATH_TO_YML** (optional\*): The path to the integration yml file that is to be published.
  See our [docs](integrations.mdx#exporting-an-integration) for more information on how to export an integration.
...
- **INTEGRATION_ID** (required): The ID of the integration to be published.
  The integration must be [imported _manually_](integrations.mdx#importing-an-integration) once into the production tenant before it can be imported automatically with this action.
