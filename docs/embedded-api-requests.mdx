...
This article outlines how to fetch data from the Prismatic API using the embedded SDK.
...
For an example of querying the Prismatic API to generate a custom marketplace UI, see [GitHub](https://github.com/prismatic-io/embedded/blob/main/example-embedded-app/pages/examples/custom-ui-elements.tsx).

## The Prismatic GraphQL API

Prismatic's API is GraphQL-based.
Full documentation on the API is available in the [API docs](api/api-overview.mdx), including several [example queries and mutations](api/using-prismatic-api.mdx).
You can try out the Prismatic API using our [GraphiQL explorer tool](/explorer/).

## Embedded user scope and API permissions

When you authenticate a customer user in the embedded SDK, that user is attached to one of your customers in Prismatic.
