...
Integrations in Prismatic are largely built by stringing components together. Built-in components (like [SFTP](/docs/components/sftp/)) can be used in any number of integrations, but so can each of your custom components.
...
The good thing is that all the components (whether built-in or custom) are versioned. Each time you run `prism components:publish` on one of your custom components, Prismatic publishes a new version of the component as V2, V3, etc.
