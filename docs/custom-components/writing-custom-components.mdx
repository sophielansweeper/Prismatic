...
Prismatic is extensible and allows for developer users to develop their own custom components.
Components that Prismatic users develop are proprietary to their [organization](organization.mdx), and are private.
...
Prismatic provides a NodeJS package, [@prismatic-io/spectral](https://www.npmjs.com/package/@prismatic-io/spectral), which provides TypeScript typing and some utility functions.
...
If you do not have Prismatic's CLI tool, `prism`, installed, please take a moment to look through the [Prism overview page](cli/cli-usage.mdx).
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Step Config - Properly Format Name in Prismatic app"
/>
...
You can leverage the `util.types.toNumber()` utility function and `clean` property to ensure that the input is presented to the `perform` function as a number:
...
- **string** will allow users to input or reference a string of characters.
  <Screenshot
    ...
    maxWidth="500px"
    alt="String input in Prismatic app"
  />
- **password** will allow users to input or reference a string of characters, and the string will be obfuscated in the UI.
  <Screenshot
    ...
    maxWidth="500px"
    alt="Password input in Prismatic app"
  />
- **boolean** allows users to enter one of two values: true or false.
  <Screenshot
    ...
    maxWidth="500px"
    alt="Boolean input in Prismatic app"
  />
- **code** opens a code editor so users can enter XML, HTML, JSON, etc.
  ...
  <Screenshot
    ...
    maxWidth="600px"
    alt="Code editor in Prismatic app"
  />
- **conditional** allows users to enter a series of logical conditionals.
  ...
  <Screenshot
    ...
    maxWidth="500px"
    alt="Conditional input in Prismatic app"
  />
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Dropdown menu in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Value List collection in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="500px"
  alt="Key Value List Collection in Prismatic app"
/>
...
Prismatic provides a variety of type check and type coercion functions for common types (number, integer, string, boolean, etc).
...
<Screenshot
  ...
  maxWidth="600px"
  alt="Step results from an action in Prismatic app"
/>
...
If the final step of the integration is a [Stop Execution](components/stop-execution.mdx) action, or any custom component action that returns a `statusCode`, the redirect does not occur.
Instead, the caller receives an HTTP response with the `statusCode` specified.

Read more about [HTTP status codes for synchronous integrations](integration-triggers.mdx#http-status-codes-for-synchronous-integrations).
...
Within your custom component you can throw a `ConnectionError` in order to signal to Prismatic that there is something wrong with the connection (unable to connect to endpoint, invalid credentials, etc).
...
Some APIs (like Prismatic's!) use GraphQL.
...
- If you are working with an HTTP-based **REST API**, we recommend using Spectral's built-in `createClient` function, which creates an [Axios](https://axios-http.com/docs/intro) HTTP client behind the scenes with some useful settings preconfigured (see [example](#using-the-built-in-createclient-http-client) below).
...
Prismatic's Spectral library provides some utility functions to make writing unit tests easier.
...
In addition to step results, the test utility's `invoke` function returns an object, `loggerMock`, with information on what was logged during the action invocation.
...
- `prism components:dev:run` fetches an integration's active connection and saves the fields as an environment variable so you can run [unit tests](#unit-tests-for-custom-components) and other commands locally.
  This is helpful, since many unit tests require an access token from a validated OAuth 2.0 connection - this provides a way of fetching the token from a connection you've already authenticated in the Prismatic integration designer.
- `prism components:dev:test` publishes your component under a temporary name, and runs a single-action test integration for you that tests the action.
  This is helpful for quickly testing an action in the real Prismatic integration runner environment.

### Access connections for local testing

The `prism components:dev:run` command fetches an active connection from the Prismatic integration designer, so you can use the connection's fields for unit testing.
...
The vast majority of integrations built in Prismatic start with a schedule trigger or webhook trigger.
...
Testing a trigger is similar to [testing an action](#testing-component-actions), except you use the `harness.trigger` function instead.
...
Prismatic offers a <a href='/jsonforms/playground/'>JSON Forms playground</a> where you can create new forms and see how they would be rendered in Prismatic.
...
Integrations in Prismatic generally process serialized JSON, XML or other simple strings and pass deserialized JavaScript objects between steps.
