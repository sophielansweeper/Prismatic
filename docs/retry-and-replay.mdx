...
Not all systems are accessible all the time. So, when it makes sense, you should [build a retry function](building-integrations.mdx#integration-retry-configuration) into an integration. Retry allows the system to automatically resend the payload to the destination system every few minutes or even according to a geometric progression (retry in 2 minutes, 4 minutes after that, 8 minutes after that, etc.) Setting the retry in the integration ensures that your integration is still likely to run successfully if the destination system or something in the integration suffers a temporary outage.
...
If, however, the error persists (for example, the destination system appears to be up, but the integration keeps throwing some type of internal server error message), you may need to move up [from the retry function to the replay function](managing-instances.mdx#instance-execution-retry-and-replay).
...
- A third-party API might be unavailable when your instance tries to access it.
  That might be remedied through instance [retry](managing-instances.mdx#instance-execution-retry-and-replay), but retry doesn't handle the scenario that the API is down for hours or days at a time.
...
[Replay](managing-instances.mdx#instance-execution-retry-and-replay) allows you to run a previous execution's data through your instance again, and it's easy to do through [Prismatic's GraphQL API](api/api-overview.mdx).
...
For more information on the Prismatic API, see the [API docs](api/api-overview.mdx).
