...
For non-JSON text data, like XML, CSV, etc., you can use the [Change Data Format](components/change-data-format.mdx) component to serialize data into the appropriate format.
...
While you could specify query parameters in the URL input through a [template input](building-integrations.mdx#templates-inputs), string concatenation is prone to encoding issues.
...
For an example raw request action, see our [GitHub examples repo](https://github.com/prismatic-io/examples/blob/main/components/asana/src/actions/rawRequest.ts) for the code that backs our Asana component's [raw request action](components/asana.mdx#raw-request).

Prismatic components use a standard inputs and `sendRawRequest` function to build built-in raw request actions.
...
Some APIs, like [Fluent Commerce](components/fluent-commerce.mdx), are GraphQL-based.
