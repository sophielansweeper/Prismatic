...
You will likely have integration logic that can't be solved using the [standard components](components/component-catalog.mdx) Prismatic provides.
...
<Screenshot
  ...
  maxWidth="650px"
  alt="Reference integration trigger payload in Prismatic app"
/>
...
<Screenshot
  ...
  maxWidth="550px"
  alt="Test runner step results in Prismatic app"
/>

## Persisted data in a code step

Like a custom component, a code step can save and reference [persisted state](custom-components/writing-custom-components.mdx#execution-instance-and-cross-flow-state) at the flow (`instanceState`), cross-flow (`crossFlowState`), integration (`integrationState`) or execution (`executionState`) level.
...
<Screenshot
  ...
  alt="Use output for prior step for input to new step in Prismatic app"
/>
...
Prismatic's SDK, [@prismatic-io/spectral](https://www.npmjs.com/package/@prismatic-io/spectral) is automatically imported into each code block as `spectral`.
