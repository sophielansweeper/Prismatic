...
Check out [this article](getting-started/first-custom-component/set-up-environment.mdx) for a detailed guide on setting up your development environment.
...
This version is accessible in the Prismatic API as the integration's `externalVersion` property.
...
The functions work the same as custom trigger functions, which are documented in the [Writing Custom Components](custom-components/writing-custom-components.mdx#instance-deploy-and-delete-events-for-triggers) article.
...
```typescript {26} title="Reference the trigger payload in the onExecution function"
import { createSlackClient } from "../slackClient";
...
Connection definitions in CNI are very similar to [custom component connection definitions](custom-components/writing-custom-components.mdx#adding-connections), but you use the `connectionConfigVar` function instead of the `connection` function.
...
Data sources are defined in CNI similar to how they are defined in [custom components](custom-components/writing-custom-components.mdx#writing-data-sources), but you use the `dataSourceConfigVar` function rather than the `dataSource` function.
...
You can build advanced UI elements, like field mappers, into your config wizard using [JSON Forms data sources](jsonforms.mdx).
...
If your integration relies on [user-level config](user-level-configuration.mdx), you can add a user-level config wizard similar to how you create the integration's config wizard.
Within `configPages.ts` create a `userLevelConfigPages` object that has the same shape as `configPages`:

```typescript title="User-level config wizard"
...
```typescript title="Including user-level config in your component" {3,6,15}
...
This can be done with or without a [preprocess flow](endpoint-configuration.mdx#instance-specific-endpoint-with-a-preprocess-flow), and both methods are described below.

Full documentation on endpoint configuration is available in the [Endpoint Configuration](endpoint-configuration.mdx) article.
...
```typescript {8-11} title="Instance specific endpoint configuration without a preprocess flow"
export default integration({
  name: "shared-endpoint-example",
  description: "Shared Endpoint Example",
  iconPath: "icon.png",
...
```typescript {8-12} title="Shared endpoint configuration without a preprocess flow"
export default integration({
  name: "shared-endpoint-example",
  description: "Shared Endpoint Example",
  iconPath: "icon.png",
...
```typescript {33-36,47-52} title="Shared endpoint configuration with a preprocess flow"
import axios from "axios";
import { flow } from "@prismatic-io/spectral";
...
Prismatic provides a number of [existing components](components/component-catalog.mdx) that you can use in your code-native integrations.
...
When you initialized your code-native integration, an `.npmrc` file was created which read:
...
:::tip Yarn 2+ configuration

NPM and Yarn "Classic" (1.x) both respect a `.npmrc` file.
If you are using a more recent version of `yarn`, please replace the `.npmrc` file with a `.yarnrc.yml` file that reads:
...
```bash title="Add the Slack component's manifest to a code-native project"
...
1. You can abstract the logic from the custom component into a distinct package, and reference that logic in both the custom component and code-native integration.
   In this case, your code-native integration won't depend on a custom component - it'll run utility and helper functions from your abstracted library.
2. You can generate a component manifest, like those created for Prismatic-provided public components.
   To create your own component manifest, build and publish your component and then run:

   ```bash title="Generate a component manifest from a private component"
   npx @prismatic-io/spectral@latest component-manifest
   ```

   This will generate a manifest project alongside your component project.
   You can save this project alongside your code-native project, or publish it to a package repository that you control.
...
After adding an existing component's manifest to your code-native project, you can use a component's connection in your code-native integration's `configPages` definition.
At the top of your config pages file, import the `connectionConfigVar` function from `@prismatic-io/spectral`.
Within a config page, include a `connectionConfigVar()` that includes a `connection` property:
...
You can reference an [connection template](connections.mdx#referencing-a-connection-template-in-an-integration) in a referenced connection if you provide a `template` name value to your connection definition.

### Using existing data sources in code-native

After adding an existing component's manifest to your code-native project, you can use a component's data source in your code-native integration's `configPages` definition.
At the top of your config pages file, import the `dataSourceConfigVar` function from `@prismatic-io/spectral`.
Within a config page, include a `dataSourceConfigVar()` that includes a `dataSource` property:
...
After importing an existing component's manifest and adding it to your `componentManifests` export, you can call one of the component's actions from within your `flow`.
Components are accessible through the `context.components` object.

For example,

```typescript title="Using an existing action in a code-native integration" {6-10}
export const existingComponentTriggerFlow = flow({
...
<Screenshot
  ...
  alt="Testing a code-native integration in the Prismatic runner"
/>
...
You can also write unit tests for your code-native integration, similar to [unit tests for custom components](quickstarts/unit-testing-custom-components.mdx).
...
:::caution Testing code-native integrations with component references
...
If your integration includes an OAuth 2.0 connection, you can use the same strategy outlined in the [Unit Testing Custom Components](quickstarts/unit-testing-custom-components.mdx#option-2---pull-the-connection-information-from-an-existing-connection-in-prismatic) guide.
